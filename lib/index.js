...
var book = {
  ...
  issue : process.argv[2] || (function(){
    ...
    var issue1 = new Date();
    issue1.setHours(12,0,0,0);    // Time of a new issue being published
    issue1.setMonth(0)            // They started with #1 on (Wednesday)
    issue1.setDate(0)
    while (issue1.getDay() !== 3)
      issue1.setDate(issue1.getDate() + 1);
    var week = Math.ceil(((today.getTime() -
                           issue1.getTime()) / 86400000) / 7);
    return week;
  })(),
  pages : []
};
