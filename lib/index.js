...
function notReady(soon){
  ...
  if (soon)
    ...
    console.log('=========================')
    console.log('To download another issue than the one from this week,')
    console.log('provide this program with the issue number, e.g.:');
    console.log(process.argv[0], process.argv[1], '2015/17_18 (for a "dubbeldik nummer") or:');
    console.log(process.argv[0], process.argv[1], '1994/7 (the oldest Groene available at the time of writing)');
  }
  process.exit(11);
}
  ...
  myPeepub.create(tmppath)
    .then(function(filePath){
      cp(filePath, newpath, function(err){
        if (err)
          console.error(`Error copying ${filePath} to ${newpath}`)
        else
